...
def main():
    ...
    parser.add_argument(
        "--video-process-timeout",
        type=int,
        default=300,
        help="Seconds to wait for video processing after upload (default: 300)",
    )
    ...
    # --- Validate title length (XHS limit: 38 display-width chars) ---
    def _display_width(s: str) -> int:
        """Calculate display width: CJK/fullwidth=2, ASCII=1, emoji=2."""
        w = 0
        for ch in s:
            cp = ord(ch)
            if (0x4E00 <= cp <= 0x9FFF or 0x3400 <= cp <= 0x4DBF  # CJK Unified
    ...
    XHS_TITLE_LIMIT = 38
    XHS_CONTENT_LIMIT = 1000
    XHS_TOPIC_LIMIT = 10

    title_width = _display_width(title)
    if title_width > XHS_TITLE_LIMIT:
        print(
            f"Error: title display width {title_width} exceeds XHS limit "
            f"{XHS_TITLE_LIMIT}. Title: {title}",
            file=sys.stderr,
        )
        sys.exit(2)
    else:
        print(f"[pipeline] Title width: {title_width}/{XHS_TITLE_LIMIT} OK")
    ...
    if content_len > XHS_CONTENT_LIMIT:
        print(
            f"Error: content length {content_len} exceeds XHS limit "
            f"{XHS_CONTENT_LIMIT}. Please shorten the content.",
            file=sys.stderr,
        )
        sys.exit(2)
    else:
        print(f"[pipeline] Content length: {content_len}/{XHS_CONTENT_LIMIT} OK")
    ...
    # --- Validate topic count (XHS limit: 10 topics) ---
    if len(topic_tags) > XHS_TOPIC_LIMIT:
        print(
            f"Error: topic count {len(topic_tags)} exceeds XHS limit "
            f"{XHS_TOPIC_LIMIT}. Topics: {' '.join(topic_tags)}",
            file=sys.stderr,
        )
        sys.exit(2)
    else:
        print(f"[pipeline] Topic count: {len(topic_tags)}/{XHS_TOPIC_LIMIT} OK")
    ...
    try:
        if is_video_mode:
            publisher.publish_video(
                ...
                video_process_timeout=args.video_process_timeout,
            )
        else:
            publisher.publish(
    ...
    if should_publish:
        ...
        try:
            result = publisher._click_publish(post_time != None)
            # result is now a dict: {status: str, link: str|None}
            if isinstance(result, dict):
                status = result.get("status", "unknown")
                link = result.get("link")
            else:
                # backward compat: old return was string|None
                status = "published" if result else "unknown"
                link = result
            if status == "published":
                print("PUBLISH_STATUS: PUBLISHED")
                if link:
                    print(f"[pipeline] Note published at: {link}")
                else:
                    print("[pipeline] Published but note link not found on page.")
                import time
                import random
                wait_seconds = 0.3 + random.random() * 0.2  # 0.3-0.5 seconds
                time.sleep(wait_seconds)
                print(f"[pipeline] Waiting {wait_seconds:.1f}s before closing tab...")
                publisher.close_tab()
            elif status == "draft":
                print("PUBLISH_STATUS: DRAFT")
                print("[pipeline] Note was saved to drafts instead of publishing. Check creator center.")
            elif status == "error":
                print("PUBLISH_STATUS: ERROR")
                print("[pipeline] Publish failed with error. Check creator center.")
                if downloader:
                    downloader.cleanup()
                sys.exit(2)
            else:
                print("PUBLISH_STATUS: UNKNOWN")
                print("[pipeline] Could not confirm publish result. Check drafts / creator center.")
        except CDPError as e:
