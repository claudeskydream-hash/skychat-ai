...
# 注意: 不能用根域名 "https://creator.xiaohongshu.com" — 即使已登录,
# 服务端也会把根路径 / 重定向到 /login (根路径没业务内容). 必须用一个
# "已登录则不跳, 未登录才跳 /login" 的深链, check_login 的 URL 判定才准.
# 2026-05-26 实测: 根路径任何情况下都跳 /login (误判源); /new/home 登录则不跳.
XHS_CREATOR_LOGIN_CHECK_URL = "https://creator.xiaohongshu.com/new/home"
...
SELECTORS = {
    ...
    # Publish button (XHS frequently changes button class names)
    "publish_button": ".publish-page-publish-btn button.bg-red",
    "publish_button_text": "发布笔记",
    "publish_button_text_alt": "发布",
...
VIDEO_PROCESS_TIMEOUT = 300  # seconds to wait for video processing (5 min default)
...
class XiaohongshuPublisher:
    ...
    def close_tab(self):
        ...
        if not self.tab_id:
            return
        try:
            self._send("Page.close")
            print(f"[cdp_publish] Tab {self.tab_id} closed.")
            self.tab_id = None
        except CDPError as e:
            print(f"[cdp_publish] Failed to close tab: {e}")
    ...
    def _count_uploaded_images(self) -> int:
        ...
        Tries several known selectors and also scans inline
        ``background-image`` styles, which Xiaohongshu commonly uses to
        render preview thumbnails. Stays resilient to creator-page
        DOM changes by combining multiple strategies.
        """
        count = self._evaluate(f"""
            (() => {{
                const selectors = [
                    ...
                    "[class*='upload-wrapper'] [class*='preview']",
                ...
                // Fallback: count elements rendered via background-image
                // (xhs preview thumbs are often <div style="background-image:url(blob:...)">).
                try {{
                    const bgItems = Array.from(
                        ...
                        if (!bg) return false;
                        return bg.includes('blob:')
                            || bg.includes('xhscdn')
                            || bg.includes('data:image');
                    }});
                    maxCount = Math.max(maxCount, bgItems.length);
                }} catch (error) {{}}
                return maxCount;
            }})()
        """)
    ...
    def _editor_ready(self) -> bool:
        """Return True once the title+content editor area is visible.

        Used as a fallback signal that the image-upload step has
        succeeded even when the preview-count selectors fail to match.
        """
        ...
        result = self._evaluate(f"""
            (() => {{
                try {{
                    const hasTitle = !!document.querySelector({json.dumps(title_selectors)});
                    const hasEditor = !!document.querySelector({json.dumps(content_selectors)});
    ...
    def _wait_for_uploaded_images(self, expected_count: int, timeout_seconds: float = 60.0):
        ...
        Fallback path: when the preview-count selectors no longer match
        (xhs page revamp), but the title+content editor area is visible
        and stays visible for ~2s, treat the upload as succeeded so the
        publish flow keeps moving instead of timing out.
        """
        deadline = time.time() + max(5.0, float(timeout_seconds))
        last_count = -1
        editor_fallback_announced = False
        while time.time() < deadline:
            ...
            if current_count >= expected_count:
                return
            # Fallback: editor visible ⇒ upload step has progressed.
            if self._editor_ready():
                if not editor_fallback_announced:
                    print(
                        "[cdp_publish] Preview count not reached "
                        f"({current_count}/{expected_count}), but the editor "
                        "has appeared. Re-checking after 2s before accepting "
                        "this as success (xhs preview DOM may have changed)."
                    )
                    editor_fallback_announced = True
                self._sleep(2.0, minimum_seconds=1.0)
                if self._editor_ready():
                    print(
                        "[cdp_publish] Editor still ready — proceeding "
                        "without confirmed preview count."
                    )
                    return
    ...
    def _get_shadow_publish_btn_rect(self) -> dict[str, Any] | None:
        """Locate the publish button inside xhs-publish-btn's closed shadow DOM via CDP.

        XHS now renders the publish/save-draft buttons as a web component
        (<xhs-publish-btn>) with a closed shadow root. Standard querySelector
        cannot access closed shadow roots, so we use CDP DOM commands to
        navigate into the shadow tree and find button.ce-btn.bg-red.
        """
        try:
            search = self._send("DOM.performSearch", {
                "query": "xhs-publish-btn",
                "includeUserAgentShadowDOM": True
            })
            if not search.get("resultCount"):
                return None
            nodes = self._send("DOM.getSearchResults", {
                "searchId": search["searchId"],
                "fromIndex": 0,
                "toIndex": 1,
            })
            btn_nids = nodes.get("nodeIds", [])
            if not btn_nids:
                return None

            node_info = self._send("DOM.describeNode", {
                "nodeId": btn_nids[0],
                "depth": 2,
                "pierce": True,
            })
            shadow_roots = node_info.get("node", {}).get("shadowRoots", [])
            if not shadow_roots:
                return None
            sr_nid = shadow_roots[0].get("nodeId")
            if not sr_nid:
                return None

            sr_btns = self._send("DOM.querySelectorAll", {
                "nodeId": sr_nid,
                "selector": "button.ce-btn.bg-red, button[class*='bg-red']",
            })
            sr_btn_ids = sr_btns.get("nodeIds", [])
            if not sr_btn_ids:
                return None

            resolved = self._send("DOM.resolveNode", {"nodeId": sr_btn_ids[0]})
            obj_id = resolved.get("object", {}).get("objectId")
            if not obj_id:
                return None

            rect_result = self._send("Runtime.callFunctionOn", {
                "objectId": obj_id,
                "functionDeclaration": (
                    "function() {"
                    " const r = this.getBoundingClientRect();"
                    " const dis = this.hasAttribute('disabled')"
                    "   || String(this.className).includes('disabled');"
                    " return {x: r.x, y: r.y, width: r.width, height: r.height, disabled: dis};"
                    "}"
                ),
                "returnByValue": True,
            })
            rect = rect_result.get("result", {}).get("value")
            if rect and rect.get("width", 0) > 0 and not rect.get("disabled"):
                return rect
            return None
        except Exception:
            return None
    ...
    def _get_publish_button_rect(self) -> dict[str, Any] | None:
        ...
        # Try the modern xhs-publish-btn web component approach first
        shadow_rect = self._get_shadow_publish_btn_rect()
        if shadow_rect:
            return shadow_rect
        return self._evaluate(f"""
            (() => {{
                ...
                const keywords = [
                    ...
                    {json.dumps(SELECTORS.get("publish_button_text_alt", "发布"))},
                ...
                for (const node of buttons) {{
                    ...
                    // Skip left sidebar navigation area
                    if (r.x < 250 && r.y < 300) continue;
    ...
    def _is_publish_button_ready(self) -> bool:
        ...
        # Fast-path: check the modern xhs-publish-btn shadow DOM component
        if self._get_shadow_publish_btn_rect() is not None:
            return True
        ready = self._evaluate(f"""
            (() => {{
                ...
                // Strategy 1: known CSS selectors (not div.btn-inner — that matches the sidebar nav)
                const selectors = [
                ...
                // Strategy 2: text match — but skip elements in the left nav (x < 250 or y < 300)
                const keywords = [
                    {json.dumps(SELECTORS["publish_button_text"])},
                    {json.dumps(SELECTORS.get("publish_button_text_alt", "发布"))},
                    {json.dumps(SELECTORS["schedule_publish_button_text"])},
                ...
                for (const node of candidates) {{
                    ...
                    const rect = node.getBoundingClientRect();
                    // Skip left sidebar navigation area
                    if (rect.x < 250 && rect.y < 300) continue;
                    const text = (node.innerText || node.textContent || "").replace(/\\s+/g, " ").trim();
    ...
    def _click_tab(self, tab_selector: str, tab_text: str):
        ...
        1. JavaScript locates the visible tab element and returns its center coordinates.
        2. CDP Input.dispatchMouseEvent simulates a real mouse click at those coordinates.

        This is necessary because el.click() (synthetic JS event) does not trigger
        the SPA's Vue event handlers on the tab elements.
        """
        ...
        # Step 1: Find visible tab and return its center coordinates.
        # Retry up to ~12 s to allow Vue SPA to finish rendering tabs after navigation.
        locate_js = f"""
            (function() {{
                ...
                function isVisible(el) {{
                    ...
                    var style = el.style || {{}};
                    if (style.left && style.left.indexOf('-9999') !== -1) return false;
                    if (style.top && style.top.indexOf('-9999') !== -1) return false;
                    if (style.opacity && parseFloat(style.opacity) < 0.01) return false;
                    var rect = el.getBoundingClientRect();
                    if (rect.width <= 0 || rect.height <= 0) return false;
                    // Exclude off-screen copies (creator tabs render a duplicate at
                    // ~(-9710, -9910) that getBoundingClientRect returns negative coords for).
                    // 2026-05-26: this is what was causing 'Could not find 上传图文 tab' —
                    // querySelectorAll DOM order put the off-screen duplicate first and the
                    // old check (style.left/top containing literal '-9999') missed it because
                    // the offset is applied via class, not inline style.
                ...
                var selectors = ['{tab_selector}', {selector_alt_literal}];
                for (var s = 0; s < selectors.length; s++) {{
                    var tabs = document.querySelectorAll(selectors[s]);
                    for (var i = 0; i < tabs.length; i++) {{
                        if (matches(tabs[i].textContent) && isVisible(tabs[i])) {{
        ...
        coords_json = self._evaluate(locate_js)
        if not coords_json:
            # SPA may still be rendering — retry for up to 12 s before giving up.
            locate_deadline = time.time() + 12
            retry_num = 0
            while not coords_json and time.time() < locate_deadline:
                retry_num += 1
                print(
                    f"[cdp_publish] '{tab_text}' tab not found yet, "
                    f"waiting for SPA to render (retry {retry_num})..."
                )
                self._sleep(2.5, minimum_seconds=2.0)
                coords_json = self._evaluate(locate_js)

        if not coords_json:
            # Fallback: check if upload input is already ready (tab already active)
            if "图文" in tab_text:
            ...
            if "视频" in tab_text:
                # Video tab may already be active (it's the default on some page states).
                video_input_ready = self._evaluate(
                    "!!document.querySelector('input[type=\"file\"][accept*=\"video\"]') || "
                    "!!document.querySelector('.upload-video input[type=\"file\"]') || "
                    "!!document.querySelector('input[accept*=\"mp4\"]')"
                )
                if video_input_ready:
                    print(
                        "[cdp_publish] '上传视频' tab not found, but video upload input is ready. "
                        "Continuing..."
                    )
                    return
        ...
        # Step 2: Simulate real mouse click via CDP
        try:
            coords = json.loads(coords_json)
            x, y = coords["x"], coords["y"]
            print(f"[cdp_publish] Clicking '{tab_text}' tab at ({x}, {y}) via CDP mouse event...")
            ...
            raise CDPError(f"Failed to parse tab coordinates: {e}")
    ...
    def _wait_video_processing(self, timeout_seconds: float = VIDEO_PROCESS_TIMEOUT):
        ...
        print(f"[cdp_publish] Waiting for video processing to complete (timeout={int(timeout_seconds)}s)...")
        ...
        while time.time() < deadline:
            ...
            if pct and pct != last_pct:
                ...
                print(f"[cdp_publish] Video processing: {pct}  (剩余 {remaining}s)")
                last_pct = pct

            time.sleep(VIDEO_PROCESS_POLL)

        raise CDPError(
            f"Video processing did not complete within {int(timeout_seconds)}s. "
            "The video may be too large or processing is slow. "
            "Try using --video-process-timeout to increase the timeout."
        )
    ...
    def _click_publish(self, scheduled: bool = False):
        ...
        # Wait for publish result - check for success, draft, or failure
        publish_result = self._wait_for_publish_result()
        return publish_result

    def _wait_for_publish_result(self, timeout_seconds=30.0):
        """Wait for publish result after clicking publish button.

        Returns dict with:
          - status: "published" | "draft" | "error" | "unknown"
          - link: URL if published, else None
        """
        print("[cdp_publish] Waiting for publish result...")
        deadline = time.time() + timeout_seconds
        while time.time() < deadline:
            self._sleep(2, minimum_seconds=1.0)
            result = self._evaluate("""(function() {
                ...
                if (bodyText.includes('发布失败') || bodyText.includes('请修改')) return JSON.stringify({status:"error",link:null});
                return JSON.stringify({status:"pending",link:null});
            })()""")
            try:
                import json as _json
                data = _json.loads(result) if isinstance(result, str) else result
            except Exception:
                data = {"status": "pending", "link": None}
            status = data.get("status", "pending")
            if status == "published":
                print(f"[cdp_publish] Publish confirmed! link={data.get('link')}")
                return data
            elif status == "draft":
                print("[cdp_publish] Note was saved as DRAFT, not published.")
                return data
            elif status == "error":
                print("[cdp_publish] Publish failed with error on page.")
                return data
        print("[cdp_publish] Timeout waiting for publish result.")
    ...
    def publish_video(
        ...
        content: str,
        video_path: str,
        video_process_timeout: float = VIDEO_PROCESS_TIMEOUT,
    ):
        """
        ...
        Args:
            ...
            video_process_timeout: Seconds to wait for video processing (default 300s)
        """
        if not self.ws:
            raise CDPError("Not connected. Call connect() first.")
        ...
        self._wait_video_processing(timeout_seconds=video_process_timeout)
